bs.one.samp.dist <- function(dat, N = 1e4)
{
  ...
  hist(sam.mean,freq = FALSE,breaks = 25, 
       ...
       xlab = paste( "Data: n =", n, " , mean =", 
                     signif(mean(dat) , digits = 5), " , se =", 
                     signif(sd(dat)/sqrt(n)), digits = 5))
  points(density(sam.mean), type = "l")
  
  x <- seq(min(sam.mean), max(sam.mean), length = 1000)
  points(x, dnorm(x,mean = mean(dat),
                  sd = sd(dat)/sqrt(n)),
...
t.dist.pval<- function(t.summary) {
  ...
  x.curve <- seq(lim.lower,
                 lim.upper,
                 length = 200)
  y.curve <- dt(x.curve,
                df = t.summary$parameter)
